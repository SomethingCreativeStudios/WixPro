        ...
        public static List<WixossCard> FindCards(WixCardSearchModel searchCard, SortBy sortBy, SortOrder sortOrder)
        {
            ...
            foreach ( var wixCard in totalCards)
            {
                ...
                if ( addCard )
                {
                    ...
                    if ( !CheckEnum<WixossCard.CardColor>(searchCard.Color , wixCard.Color) )
                        addCard = false;

                    if ( !CheckEnum<WixossCard.CardTiming>(searchCard.Timing , wixCard.Timing) )
                        addCard = false;

                    if ( (searchCard.Type != null && searchCard.Type != WixossCard.CardType.NoType) && searchCard.Type.Value != wixCard.Type )
                        addCard = false;
